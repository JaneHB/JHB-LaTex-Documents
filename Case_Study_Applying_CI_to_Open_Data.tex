...
The case study will be conducted following the methodology of Robert Yin, the leading authority on case study methodology \citep{Yin_CS_Design2013}. %SF: You can just say you're following Yin's methodology. You can't guarantee his role as 'leading authority' won't be usurped, so best to justify methodology on the basis of successful case study research rather than an appeal to authority.
This involves ensuring five areas are included in the methodology \citep{Yin_Case_studies_and_org_in_1985}:
...
Thus, a UK Local Authority who currently publish date open source was approached to establish whether they would agree to work with the research team in applying the CI framework to real data, in a real practical setting. They kindly agreed to collaborate and trial the framework.  %SF: You should use 'we' in the royal sense rather than 'research team' in the dissertation as this is part of your doctoral research

\section {Research Design - Protocol}

\subsection {Hypothesis}
%SF: May be worth boxing the hypotheses or emphasising them in some way
First, it is hypothesised that Existing guidelines do not provide guidance on privacy beyond advising that data protection regulations must be adhered to, hence the reason for this case study. % SF: ref to this hypothesis would be useful.  If the claim is verbatim then consider marking up the text with inverted commas, i.e. ``claim'' or italics, i.e. \emph{claim}
Then, it is hypothesised that Nissenbaum's CI framework, whilst it may be applied to any scenario, is best suited to a specific case where all but will not prove sufficient when the data recipient is not known, thereby severely limiting the usability of the framework in practice.  % SF: Use the active voice where possible, i.e. We claim Nissenbaum's CI framework.. rather than 'It is hypothesised that Nissenbaum's CI framework...
...
\begin{enumerate}
\item No practical advice how to incorporate privacy into decision making around open source publishing, beyond stipulation that data protection regulations must be adhered to, can be found in existing guidelines? %SF: is this a question?
...
\item Using the CI framework in its current format result in most datasets being rejected for publication? %SF: avoid closed questions.  What about 'how well does Nissenbaum's CI framework work in deciding.....'
\item Does using the CI framework in its current format result in public bodies not being able to meet their legal obligations with regard to making data available open source? %SF: See comment above re: open and closed questions.
\end{enumerate}

\subsection {Question Preparation}

In order to answer the research questions relating to Nissenbaum's CI framework, it is first necessary to create an easy to follow methodology for following the framework. This will be done in three phases:  %:SF: What makes this 'easy to follow'?  If you can't qualify this then I would suggest avoiding this adjective entirely.
...
This section explains how Nissenbaum's CI framework has been interpreted and applied into a Practical Application Model (PAM) by the primary researcher. %SF: What is a PAM? 
...
Upon studying Nissenbaum's book in detail, %SF: unnecessary commentary
it was determined that there are two areas where Nissenbaum offers some level of detailed guidance for applying the framework to a practical in the book. These are; %SF: consider summarising and itemising this guidance if you can.
...
In her book, Nissenbaum explains that the framework has three key elements; explanation, evaluation and prescription (p. 190). However, it was felt that practitioners may not relate to these descriptions as it is not necessarily obvious to a layman what they mean. %SF: Some examples of where practitioners may not relate to these descriptions would be useful.
...
As it appeared that these elements provided an excellent, %SF: can you qualify 'excellence'?  If not remove this adjective.
...
This, it is contended, % SF: contended by you?  If, then say 'we contend'
...
PAM will be presented to the practitioner in an excel spreadsheet consisting of three worksheets, one for each step. %SF: Why an Excel spreadsheet.  I would suggest just saying 'spreadsheet'
...
%SF: I think scoring needs to be spelt out in more detail going forward and, arguably, independently validated.
...
% SF: It may be worth putting the actual questions in the appendices.
...
The first part of the analysis will be a detailed analysis of the existing guidelines produced by Government bodies and affiliated parties to assist practitioners when making decisions about open source publication. This will be done following Corbin and Strauss grounded theory methodology \citep{Corbin_Strauss} to establish how much of that guidance pertains to privacy. Then, the results will be amalgamated to confirm or deny hypotheses 1.
